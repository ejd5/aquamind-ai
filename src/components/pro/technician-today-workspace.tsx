...
function writeCachedDay(interventions: InterventionRow[]) {
  try {
    ...
    // Storage may be blocked by device policy; the online route remains usable.
  }
}
...
export function TechnicianTodayWorkspace({ mobileMode = false }: { mobileMode?: boolean }) {
      ...
      <div className="space-y-3">
        {interventions.map((intervention) => {
          ...
          const clientName = [intervention.client?.firstName, intervention.client?.lastName]
            .filter(Boolean)
            .join(' ')
          const detailsHref = mobileMode
            ? `/pro/app/report?id=${encodeURIComponent(intervention.id)}`
            : `/pro/app/interventions/${intervention.id}`
