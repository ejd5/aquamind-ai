...
export function PipelineWorkspace({ mode }: { mode: 'qualification' | 'matching' }) {
  ...
  async function qualify(lead: Lead) {
    ...
    setMessage(res.ok ? 'Qualification calculée et journalisée.' : 'La qualification a échoué.'); setWorking(''); if (res.ok) void load()
  }
  ...
  async function assign(lead: Lead, userId: string) {
    ...
    setMessage(res.ok ? 'Attribution humaine confirmée et journalisée.' : 'L’attribution a échoué.'); setWorking(''); if (res.ok) void load()
  }
  ...
  return <div className="space-y-6">
    <div><span className="section-label">Growth Inbox Beta</span><h1 className="mt-2 font-display text-3xl font-bold sm:text-4xl">{mode === 'qualification' ? 'Qualification' : 'Matching humain'}</h1><p className="mt-1 text-sm text-muted-foreground">{mode === 'qualification' ? 'Priorisation déterministe des demandes entrantes consenties.' : 'Affectation à un collaborateur après validation humaine.'}</p></div>
    ...
    {loading ? <div className="flex justify-center py-14"><Loader2 className="h-6 w-6 animate-spin" /></div> : visible.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">Aucune demande dans cette file.</div> : <div className="space-y-3">{visible.map((lead) => <div key={lead.id} className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-white/40 bg-white/60 p-4 dark:border-white/10 dark:bg-white/[0.04]"><div><Link href={`/growth/app/leads/${lead.id}`} className="font-semibold hover:text-gold">{lead.firstName} {lead.lastName}</Link><p className="text-xs text-muted-foreground">{lead.serviceType || 'Service non précisé'} · {lead.city || 'Zone non précisée'} · score {lead.score}/100</p><p className="mt-1 flex items-center gap-1 text-[10px] text-emerald-700"><ShieldCheck className="h-3 w-3" />Consentement {lead.consent ? 'confirmé' : 'absent'}</p></div>{mode === 'qualification' ? <button disabled={working === lead.id || !lead.consent} onClick={() => qualify(lead)} className="inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-xs font-bold text-primary-foreground disabled:opacity-50">{working === lead.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Bot className="h-4 w-4" />}Qualifier</button> : <label className="text-xs font-semibold text-muted-foreground">Attribuer<select defaultValue={lead.assignedTo || ''} disabled={working === lead.id} onChange={(e) => void assign(lead, e.target.value)} className="ml-2 rounded-xl border border-border bg-background p-2 text-xs text-foreground"><option value="">Choisir</option>{members.filter((m) => ['owner','admin','manager','technician'].includes(m.role)).map((m) => <option key={m.id} value={m.user.id}>{m.user.name || m.user.email}</option>)}</select></label>}</div>)}</div>}
  </div>
}
