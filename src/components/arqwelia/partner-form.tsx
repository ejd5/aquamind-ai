    ...
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div>
          ...
          <input className={fieldCls} value={form.postalCode} onChange={(e) => setForm({ ...form, postalCode: e.target.value })} />
        </div>
        <div>
          <label className="mb-1.5 block text-xs font-semibold text-arq-mist/70">
      ...
      <label className="flex items-start gap-3 text-xs text-arq-mist/65">
        <input
          type="checkbox"
          checked={consent}
          onChange={(e) => setConsent(e.target.checked)}
          className="mt-0.5 h-4 w-4 rounded border-arq-mist/30 bg-arq-navy accent-arq-aqua"
          aria-invalid={!!errors.consent}
        />
        <span>{t('partner.form.consent')}</span>
      </label>
      <p className="-mt-2 text-[11px] text-arq-mist/40">{t('partner.form.consentHint')}</p>
      {errors.consent && <p className="text-xs text-red-400">{errors.consent}</p>}
      <button
        type="submit"
        disabled={status === 'loading'}
        className="inline-flex min-h-[44px] w-full items-center justify-center rounded-full bg-arq-aqua px-6 py-3 text-sm font-bold text-arq-navy transition-transform hover:scale-[1.01] disabled:opacity-50 sm:w-auto"
      >
