...
import { Sparkles, Mail, Lock, User as UserIcon, ArrowRight, ArrowLeft, Loader2, ShieldCheck, Gift, Rocket } from 'lucide-react'
...
export default function AuthPage() {
  ...
  /**
   * Demo mode — 1 clic, sans mot de passe.
   * Appelle /api/demo/login qui crée idempotemment un compte démo
   * (demo@aqwelia.app / aqwelia-demo-2026) avec un profil piscine,
   * puis se connecte avec ces identifiants.
   * Solution PERMANENTE pour les resets de sandbox.
   */
  async function handleDemo() {
    ...
    try {
      ...
      if (!result?.ok) throw new Error('Demo sign-in failed')
    ...
    <div className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md flex-col justify-center px-4 py-8">
        ...
        <div className="glass-card rounded-2xl p-6 sm:p-8">
          {/* Bouton Mode démo — PROMINENT, toujours disponible, sans mot de passe.
              Solution permanente : le compte démo est recréé idempotemment. */}
          <button
            type="button"
            onClick={handleDemo}
            disabled={loading}
            className="group mb-5 flex w-full items-center justify-center gap-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-3.5 text-sm font-bold text-white shadow-lg shadow-emerald-500/30 transition-all hover:scale-[1.01] hover:shadow-emerald-500/50 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {loading ? (
              <>
              ...
              <>
                ...
                Accès démo — 1 clic, sans mot de passe
              </>
            )}
          </button>
              ...
              <div className="grid gap-2">
                {oauthProviders.google && (
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2.5 rounded-xl border border-border bg-background px-4 py-2.5 text-sm font-semibold text-foreground shadow-sm transition-all hover:border-gold/40 hover:bg-secondary/40 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {oauthLoading === 'google' ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
                ...
                {oauthProviders.apple && (
                  <button
                    ...
                    disabled={oauthLoading !== null}
                    className="flex w-full items-center justify-center gap-2.5 rounded-xl border border-border bg-foreground px-4 py-2.5 text-sm font-semibold text-background shadow-sm transition-all hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {oauthLoading === 'apple' ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
