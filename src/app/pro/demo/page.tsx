/**
 * AQWELIA Pro — Visual demo page.
 *
 * URL: /pro/demo
 * Server component. Renders 5 sections of placeholder mockups (Dashboard,
 * Clients, Planning, Intervention, PDF Report). Each section has a textual
 * description and a stylised browser-frame placeholder.
 *
 * Final CTA → /pro/early-access.
 */
import type { Metadata } from 'next'
import Link from 'next/link'
import { ArrowRight, Sparkles, LayoutDashboard, Users, Calendar, ClipboardList, FileText } from 'lucide-react'
...
export default async function ProDemoPage() {
  ...
  const SECTIONS = [
    ...
    },
    {
      id: 'clients',
      icon: Users,
      title: t('demoSectionClients'),
      text: t('demoSectionClientsText'),
      rows: 5,
    },
    {
      id: 'planning',
      icon: Calendar,
      title: t('demoSectionPlanning'),
      text: t('demoSectionPlanningText'),
      bars: [30, 70, 50, 90, 40],
    },
    {
      id: 'intervention',
      icon: ClipboardList,
      title: t('demoSectionIntervention'),
      text: t('demoSectionInterventionText'),
      bars: [40, 65, 80, 55, 75],
    },
    {
      id: 'report',
      icon: FileText,
      title: t('demoSectionReport'),
      text: t('demoSectionReportText'),
    ...
    <div className="overflow-hidden rounded-2xl border border-white/40 bg-white/70 shadow-[0_25px_60px_-25px_oklch(0.45_0.12_195/0.3)] backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.04]">
      ...
      <div className="flex items-center gap-2 border-b border-border/40 bg-white/50 px-4 py-2.5 dark:bg-white/[0.03]">
        ...
        <div className="mx-auto rounded-md bg-white/70 px-3 py-0.5 text-[10px] font-medium text-muted-foreground dark:bg-white/10">
          aqwelia.app/pro/{label.toLowerCase().split(' ')[0]}
        </div>
      </div>
