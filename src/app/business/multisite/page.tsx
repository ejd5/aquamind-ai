/**
 * AQWELIA Business — Multi-site offer page.
 *
 * URL: /business/multisite
 * Server component. Hero + benefits + checklist + CTA.
 */
import type { Metadata } from 'next'
import Link from 'next/link'
import { ArrowRight, Sparkles, Building2, MapPin, BarChart3, ShieldCheck } from 'lucide-react'
import { getTranslations } from 'next-intl/server'

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('business')
  return {
    title: t('multisiteMetaTitle'),
    description: t('multisiteMetaDescription'),
  }
}

export default async function BusinessMultisitePage() {
  const t = await getTranslations('business')

  const BENEFITS = [
    { icon: Building2, title: t('multisiteBenefit1Title'), text: t('multisiteBenefit1Text') },
    { icon: MapPin, title: t('multisiteBenefit2Title'), text: t('multisiteBenefit2Text') },
    { icon: BarChart3, title: t('multisiteBenefit3Title'), text: t('multisiteBenefit3Text') },
    { icon: ShieldCheck, title: t('multisiteBenefit4Title'), text: t('multisiteBenefit4Text') },
    { icon: Building2, title: t('multisiteBenefit5Title'), text: t('multisiteBenefit5Text') },
    { icon: BarChart3, title: t('multisiteBenefit6Title'), text: t('multisiteBenefit6Text') },
  ]

  const CHECKLIST = t.raw('multisiteChecklist') as string[]
    ...
    <>
      <section className="relative overflow-hidden py-16 sm:py-20">
        ...
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <span className="section-label inline-block">{t('multisiteEyebrow')}</span>
