/**
 * AQWELIA Business — Conciergeries offer page.
 *
 * URL: /business/conciergeries
 * Server component. Hero + benefits + checklist + CTA.
 */
import type { Metadata } from 'next'
import Link from 'next/link'
import { ArrowRight, Sparkles, ConciergeBell, Users, Calendar, ShieldCheck } from 'lucide-react'
import { getTranslations } from 'next-intl/server'

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('business')
  return {
    title: t('conciergeriesMetaTitle'),
    description: t('conciergeriesMetaDescription'),
  }
}

export default async function BusinessConciergeriesPage() {
  const t = await getTranslations('business')

  const BENEFITS = [
    { icon: ConciergeBell, title: t('conciergeriesBenefit1Title'), text: t('conciergeriesBenefit1Text') },
    { icon: Users, title: t('conciergeriesBenefit2Title'), text: t('conciergeriesBenefit2Text') },
    { icon: Calendar, title: t('conciergeriesBenefit3Title'), text: t('conciergeriesBenefit3Text') },
    { icon: ShieldCheck, title: t('conciergeriesBenefit4Title'), text: t('conciergeriesBenefit4Text') },
    { icon: ConciergeBell, title: t('conciergeriesBenefit5Title'), text: t('conciergeriesBenefit5Text') },
    { icon: Calendar, title: t('conciergeriesBenefit6Title'), text: t('conciergeriesBenefit6Text') },
  ]

  const CHECKLIST = t.raw('conciergeriesChecklist') as string[]
        ...
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <div className="relative overflow-hidden rounded-3xl border border-gold/40 bg-white/40 p-8 backdrop-blur-xl dark:bg-white/[0.04] sm:p-10">
            ...
            <h2 className="mt-4 font-display text-2xl font-bold leading-tight tracking-tight sm:text-3xl">
              {t('conciergeriesCtaTitle')}
            </h2>
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted-foreground sm:text-base">
              {t('conciergeriesCtaSubtitle')}
            </p>
            <Link
              href="/business#contact"
