/**
 * AQWELIA Business — Hotels offer page.
 *
 * URL: /business/hotels
 * Server component. Hero + 6 hotel-specific benefits + check-list + CTA.
 */
import type { Metadata } from 'next'
import Link from 'next/link'
import { ArrowRight, Sparkles, Hotel, Star, ShieldCheck, ClipboardCheck } from 'lucide-react'
import { getTranslations } from 'next-intl/server'

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('business')
  return {
    title: t('hotelsMetaTitle'),
    description: t('hotelsMetaDescription'),
  }
}
...
export default async function BusinessHotelsPage() {
  ...
  const BENEFITS = [
    { icon: ClipboardCheck, title: t('hotelsBenefit1Title'), text: t('hotelsBenefit1Text') },
    { icon: ShieldCheck, title: t('hotelsBenefit2Title'), text: t('hotelsBenefit2Text') },
    { icon: Hotel, title: t('hotelsBenefit3Title'), text: t('hotelsBenefit3Text') },
    { icon: Star, title: t('hotelsBenefit4Title'), text: t('hotelsBenefit4Text') },
    { icon: ClipboardCheck, title: t('hotelsBenefit5Title'), text: t('hotelsBenefit5Text') },
    { icon: ShieldCheck, title: t('hotelsBenefit6Title'), text: t('hotelsBenefit6Text') },
  ]

  const CHECKLIST = t.raw('hotelsChecklist') as string[]
    ...
    <>
      <section className="relative overflow-hidden py-16 sm:py-20">
        ...
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <span className="section-label inline-block">{t('hotelsEyebrow')}</span>
