/**
 * AQWELIA Partenaires — Shared layout for /partenaires/* marketing pages.
 *
 * Server component. Renders:
 *  - a sticky brand header with logo + Partners nav (Fournisseurs /
 *    Piscinistes / Affiliation)
 *  - the page content
 *  - the AQWELIA footer (shared with the in-app experience)
 *
 * Same design system as the landing, /pro and /care pages (glassmorphism,
...
export default async function PartenairesLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV_LINKS = [
    { href: '/partenaires#fournisseurs', label: t('navFournisseurs') },
    { href: '/partenaires#piscinistes', label: t('navPiscinistes') },
    { href: '/affiliation', label: t('navAffiliation') },
  ]
    ...
    <div className="relative flex min-h-screen flex-col bg-background">
      ...
      <header className="safe-area-top sticky top-0 z-40 border-b border-gold/20 bg-background/85 backdrop-blur-2xl">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          ...
          {/* CTA Apply — always visible */}
          <Link
            href="/partenaires#postuler"
