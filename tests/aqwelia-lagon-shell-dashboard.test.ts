...
describe('LAGON VIVANT — sidebar desktop', () => {
  ...
  it('la ligne active est identifiable (ring + teinte de la famille)', () => {
    expect(shell).toContain('bg-gradient-to-r from-lagoon/15 to-aqua-vivid/10 text-foreground shadow-sm ring-1 ring-lagoon/25')
    expect(shell).toContain('ring-1 ring-champagne/30')
  })
...
describe('LAGON VIVANT — dashboard', () => {
  it('la carte Niveau 1 (gauge eau claire) porte la hairline lagoon→aqua', () => {
    expect(dashboard).toContain('<Card className="card-accent-lagon lg:col-span-1">')
  })

  it('les quick actions utilisent les icon-chips et des surfaces teintées par action', () => {
    ...
    expect(dashboard).toContain('border-lagoon/25 bg-gradient-to-br from-lagoon/10 via-card/70 to-transparent')
...
describe('LAGON VIVANT — périmètre strict', () => {
  it('les modules hors périmètre restent intacts', () => {
    ...
    const markers = ['icon-chip', 'card-accent-lagon', 'app-bg-lagon', 'btn-aqua-gradient', 'btn-champagne-gradient', 'card-premium-champagne']
