 ...
 *
 * Validates the complete incremental migration chain while preserving the
 * historical consentAt upgrade scenarios:
 *   Scenario A — empty database: deploy all, verify consentAt
 *   Scenario B — existing database: deploy baseline, add data,
 *                apply every incremental migration, verify upgrade
...
const migrationNames = () =>
  readdirSync(MIGRATIONS_DIR).filter((entry) => /^\d{14}_/.test(entry))
 ...
 * @param {string[]} names  migration dir names to include
 * @returns {{ok: boolean, output: string}}
 */
function deployMigrations(dbUrl, names) {
  ...
  try {
    ...
    writeFileSync(
      join(tmpMigrations, 'migration_lock.toml'),
      readFileSync(join(MIGRATIONS_DIR, 'migration_lock.toml')),
    )

    writeFileSync(tmpSchema, readFileSync(SCHEMA_PATH, 'utf8'))
...
function deployAll(dbUrl) {
  return deployMigrations(dbUrl, migrationNames())
}
...
async function scenarioA() {
  ...
  try {
    ...
    try {
      ...
      ok(names.includes('20260719090000_aqwelia_brain_foundation'), 'Brain foundation migration present')
      ok(names.includes('20260722090000_aqwelia_brain_index_parity'), 'Brain index migration present')
...
async function scenarioB() {
  ...
  try {
    ...
    try {
      ...
      console.log('  Step 3: Applying incremental migrations...')
      ...
      eq(migs.length, migrationNames().length, 'all versioned migrations applied')
...
async function scenarioC() {
  ...
  try {
    ...
    try {
      ...
      eq(migs.length, migrationNames().length, 'exactly all versioned migrations are recorded once')
