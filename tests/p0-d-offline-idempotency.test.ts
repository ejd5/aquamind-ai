...
describe('P0-D offline idempotency', () => {
  it('allows only the six offline mutation APIs', () => {
    ...
    expect(() =>
      normalizeOfflineTarget('https://aqwelia.test', 'https://evil.test/api/chat', 'POST'),
    ).toThrow('External replay target denied')
  })

  it('validates stable idempotency keys', () => {
    expect(validateIdempotencyKey('018f8ca1-7d2e-7b5f-a18b-010203040506')).toBe(
      '018f8ca1-7d2e-7b5f-a18b-010203040506',
    )
  ...
  it('hashes semantically identical object bodies identically', () => {
    const first = hashOfflineMutation('POST', '/api/pool/water-test', {
      ph: 7.2,
      poolId: 'p1',
    })
    const second = hashOfflineMutation('POST', '/api/pool/water-test', {
      poolId: 'p1',
      ph: 7.2,
    })
  ...
  it('releases reservations for every non-success target response', () => {
    expect(shouldReleaseReplayReservation(200)).toBe(false)
    expect(shouldReleaseReplayReservation(201)).toBe(false)
    expect(shouldReleaseReplayReservation(204)).toBe(false)
    expect(shouldReleaseReplayReservation(400)).toBe(true)
    expect(shouldReleaseReplayReservation(401)).toBe(true)
    expect(shouldReleaseReplayReservation(409)).toBe(true)
    expect(shouldReleaseReplayReservation(500)).toBe(true)
  })
  ...
  it('never deletes a reservation after the target mutation succeeds', () => {
    const route = readFileSync(
      join(process.cwd(), 'src/app/api/offline/replay/route.ts'),
      'utf8',
    )
    expect(route).toContain('targetMutationSucceeded = true')
    expect(route).toContain('if (reservationId && !targetMutationSucceeded)')
    expect(route).toContain("'Idempotency-Body-Cached': cachedBody === null ? 'false' : 'true'")
  })
})
